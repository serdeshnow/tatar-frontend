...
const colorTable = (val) => {
    switch (true) {
        case (val < 50): return {"color": '#E69281', "img": red_strike};
        case (val >= 50 && val <= 80): return {"color": '#E2C84E', "img": yellow_strike};
        case (val > 80): return {"color": '#C1CB68', "img": green_strike};
    }
  }
...
export default function ProgressBarContainer({label, value}) {
    return(
        ...
        <div style={{"display": "flex", "justifyContent": "center", "marginTop": "10px"}}>
          <img src={colorTable(value)["img"]} className='strike_img'/>
          <a href="/" style={{color: colorTable(value)["color"]}} className="upgrade_link">повысить</a>
        </div>
        </div>
    )
}